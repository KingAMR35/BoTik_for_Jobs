...
bot.set_my_commands(
    commands=[
        ...
        telebot.types.BotCommand("job_ai_quiz", "🔍🤖 Тест на знание предпочитаемой вами профессии"),
...
def info_bot(message):
    ...
    butt3 = types.InlineKeyboardButton(text="2.1", callback_data="version_2.1")
    keyboard.row(butt1)
    keyboard.row(butt2)
    keyboard.row(butt3)
...
def version_info(call):
    ...
    "version_2.1": "🚀 Версия 2.1 🎉: \n\nВышла новая команда /job_ai_search открывает дверь в мир искусственного интеллекта,\nкоторый подберёт идеально подходящую профессию лично для вас!🤖🔍\n\n"
                    "Задайте интересующие вас вопросы и позвольте нашему продвинутому алгоритму\nпроанализировать ваши предпочтения и навыки, чтобы предложить уникальную профессию,\nкоторая воплотит ваши мечты в реальность!"
                    "✨\n\nИспользуйте команду /job_ai_search и позвольте ИИ раскрыть ваш профессиональный потенциал!"
}
